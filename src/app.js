...
import { routes, publicRoutes, nonPrivateRoutes } from "./routes";
...
import "./locales/i18n";
import PrivatePagesContainer from "./global";

export const App = () => {
  // useAnalytics({ containerId: appConfig.gtmContainerId });
  useNprogress();

  const element = useRoutes([ ...routes, ...publicRoutes ]);
  const nonPrivateElement = useRoutes(nonPrivateRoutes);
